# 更新日志

## cronet版本: 143.0.7499.194

**2026/07/07**
- 新增书架“随机阅读”，会在当前书架分组内随机打开一本书；在 A 分组触发只随机 A 分组，在 B 分组触发只随机 B 分组。
- 新增书架“书目筛选”，支持按字数区间、题材关键词、排除关键词筛选，并支持按字数、书名、作者排序。
- 新增搜索结果筛选与排序，和书架使用同一套字数、题材、排序条件。
- 新增搜索结果批量加入书架，可将当前可见且未加入书架的搜索结果一次性加入书架。
- 优化搜索筛选后的加载逻辑，筛选后结果不足时会继续加载后续搜索页，减少只看到少量结果的问题。
- 新增发现分类页筛选按钮，进入发现源任意分类后可直接使用字数、题材、排序筛选。
- 优化发现分类分页加载，每次加载 5 页，兼顾筛选可用性和大列表卡顿。
- 新增“存储清理”，可扫描并清理离线章节缓存、无效书籍缓存、应用缓存、网页数据、日志、临时备份、书源大数据、本地书封面等实际文件。
- 优化数据库瘦身逻辑，改为重启后在数据库打开前清理 SQLite WAL/SHM 等临时文件，不再提供容易长时间卡顿的深度压缩入口。
- 新增“恢复默认状态”，可重启并清空书籍、书源、设置、记录、缓存和应用私有文件，回到刚安装状态，本地书文件除外。
- 内置默认书源更新，新增默认下载的书源数据；内置订阅源移除“小说拾遗”，新增源仓库订阅源并设为默认。
- 优化首次安装体验，取消初始隐私、帮助、密码等多余弹窗。
- 调整书架右上角菜单，移除“更新目录”和“远程书籍”，保留下拉刷新；“导入书单”和“导出书单”合并为“书单管理”。
- 优化添加分组/选择分组时的列表加载，减少书架 3000-4000 本书时的卡顿。
- 优化书架筛选在大书架下的执行方式，避免筛选时一次性重排造成明显卡顿。
- 默认阅读样式更新为“文字”预设：字号 34、字距 0.07、行距 0.7、段距 1.1、滚动翻页、共用布局、隐藏状态栏和导航栏，并按预设边距显示。
- 新增内置默认字体，首次安装和恢复默认后自动使用该字体作为阅读默认字体。
- 关于页面开发人员新增 LYX，原 GitHub 链接保持不变。
- 调整部分图标和菜单显示，修复“书单管理”等菜单项轻微错位，优化筛选和更新相关图标辨识度。
- 优化“其他设置”中的重复入口和文案，减少重复选项。

**2026/03/07**
- 优化代码，修复问题

**2026/03/03**
- 优化代码，修复问题
- 视频悬浮窗播放时进行系统媒体播放通知
- 净化规则使用js时支持调用java.log
- 代码编辑器搜索替换内容支持$符号
- 优化书架滚动位置记忆
- 增加搜索结果排序时对书籍分类信息进行判断
- 增加自动检查app更新功能

**2026/02/16**
- 优化代码，修复问题
- 让小说朗读走系统媒体播放通道
- 更新内置字典规则
- 新增java.refreshBookToc函数
- java.reLoginView函数增加deltaUp参数
- 新增@webjs:规则类型
- 文件类书源支持下载链接type指定文件后缀
- 提升购买按钮权限

**2026/01/31**
- 优化代码，修复一些问题
- 正文增加锁定反向横屏

**2026/01/28**
- 新增java.reLoginView()函数，刷新登录界面
- 书源发现支持更多丰富的按钮类型
- 新增java.refreshExplore()函数
- java.open函数支持打开登录界面
- 书源简介支持html标签包裹，显示html样式
- 书籍简介和字典支持gif动态图和svg图data链接
- 书籍简介和字典支持button按钮
- 支持源控制图片显示尺寸
- 书籍简介支持markdown语法编写
- 新增java.showBrowser函数，能进行半屏显示段评
- 支持图片链接click键，不推荐继续使用旧方式
- 支持双击响应段评图
- 新增chapter.update()函数
- 新增java.showPhoto函数
- 新增java.refreshContent()函数
- 支持订阅源启动页html用js返回空
- 提升webview函数获取js结果速度
- 其余优化与修复

**2026/01/13**
- 升级cronet版本至：143.0.7499.193
- 软件自定义背景图支持.9.png格式
- 背景图导入支持直接输入图片在线链接
- 主题分享支持在线背景图链接
- 背景图支持跟随主题切换
- 主题设置支持透明操作栏，提升图片背景视觉效果
- 支持分组封面自定义图片恢复默认
- 登录UI的select类型支持action键
- 提升内置浏览器打开速度（例：订阅源、段评 打开速度大概快100毫秒左右）
- 支持正文下划线设为虚线类型
- cache.get函数新增onlyDisk参数
- tts源支持jslib规则
- tts源登录界面新增java.clearTtsCache()函数
- 支持导出单个tts源
- 编辑tts源、字典规则、TXT目录规则时误触空白区域会提示保存
- 新增正文边缘点击阈值设置，防止曲面屏误触
- 实现订阅源的登录检查规则
- 在链接访问出错时，也能执行一次登录检查规则
- StrResponse对象支持callTime()获取响应时间
- 并发访问函数支持skipRateLimit参数，绕过源并发率限制
- 视频播放器支持记录函数调用时的播放进度
- 其余细节优化与bug修复


## **必读**
【温馨提醒】 *更新前一定要做好备份，以免数据丢失！*

* 阅读只是一个转码工具，不提供内容，第一次安装app，需要自己手动导入书源。
* 正文出现缺字漏字、内容缺失、排版错乱等情况，有可能是净化规则或简繁转换出现问题。
----

* [2025年日志](https://github.com/LegadoTeam/legado/blob/record2025/app/src/main/assets/updateLog.md)　
* [2023年日志](https://github.com/LegadoTeam/legado/blob/record2023/app/src/main/assets/updateLog.md)　
* [2022年日志](https://github.com/LegadoTeam/legado/blob/record2022/app/src/main/assets/updateLog.md)　
* [2021年日志](https://github.com/LegadoTeam/legado/blob/record2021/app/src/main/assets/updateLog.md)　
