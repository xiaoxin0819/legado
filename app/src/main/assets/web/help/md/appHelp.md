...
### 1. 书架如何随机阅读？
在书架页面点击右上角“**⁝**”——“**随机阅读**”即可随机打开一本书。随机范围以当前书架分组为准：在 A 分组触发只会随机 A 分组的书，在 B 分组触发只会随机 B 分组的书。

### 2. 书架如何按字数、题材筛选？
在书架页面点击右上角“**⁝**”——“**书目筛选**”，可以设置最低字数、最高字数、包含题材关键词、排除题材关键词，并选择按默认、字数、书名、作者排序。筛选条件会保存，清空后恢复显示全部书籍。

### 3. 搜索结果如何筛选和批量加入书架？
在搜索页面右上角点击“**书目筛选**”图标，可以使用和书架相同的字数、题材、排序条件。筛选后如果当前结果太少，搜索会继续加载后续结果，尽量补足可见列表。点击“**批量加入书架**”可把当前可见且未加入书架的结果一次性加入书架。

### 4. 发现分类页如何筛选？
在“**发现**”中进入任意书源的任意分类后，右上角会显示“**书目筛选**”图标，可按字数、题材和排序筛选当前分类。分类列表每次加载 5 页，筛选结果不足时会继续向后加载，兼顾检索范围和流畅度。

### 5. 书单管理在哪里？
书架右上角菜单中的“**导入书单**”和“**导出书单**”已合并为“**书单管理**”。进入后可选择导入或导出，功能不变，入口更集中。

### 7. 如何恢复到刚安装状态？
进入“**我的**”——“**存储清理**”——“**恢复默认状态**”。该功能会重启应用，并清空书籍、书源、设置、记录、缓存和应用私有文件，回到刚安装状态；本地书源文件和本地书原文件不会主动删除。操作前请先备份。
