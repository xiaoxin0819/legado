...
import io.legado.app.help.storage.MergeRestore
...
class BackupConfigFragment : PreferenceFragment(),
    ...
    private val mergeRestoreDoc = registerForActivityResult(HandleFileContract()) {
        it.uri?.let { uri ->
            waitDialog.setText(R.string.merge_restore_running)
            waitDialog.show()
            val task = Coroutine.async {
                MergeRestore.merge(appCtx, uri)
            }.onError { e ->
                AppLog.put("合并导入备份出错\n${e.localizedMessage}", e)
                appCtx.toastOnUi("合并导入备份出错\n${e.localizedMessage}")
            }.onFinally {
                waitDialog.dismiss()
            }
            waitDialog.setOnCancelListener {
    ...
    override fun onPreferenceTreeClick(preference: Preference): Boolean {
        when (preference.key) {
            ...
            "merge_restore" -> mergeRestoreFromLocal()
    ...
    private fun mergeRestoreFromLocal() {
        mergeRestoreDoc.launch {
            title = getString(R.string.merge_restore_select_file)
            mode = HandleFileContract.FILE
            allowExtensions = arrayOf("zip")
        }
    }
