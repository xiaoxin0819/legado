...
class BookshelfManageActivity :
    ...
    private var bookFilterConfig: BookFilterConfig = BookFilterConfig()
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        bookFilterConfig = BookFilterDialog.load(this, MANAGE_FILTER_PREF)
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        menu.findItem(R.id.menu_book_filter)?.isChecked = bookFilterConfig.isActive
        upMenu()
        ...
        booksFlowJob = lifecycleScope.launch {
            ...
            appDb.bookDao.flowByGroup(viewModel.groupId).map { list ->
                ...
                .conflate().collect {
                    books = it
                    upBookData()
                    itemTouchCallback.isCanDrag = bookSort == 3 && !bookFilterConfig.isActive
                }
        }
    }
    ...
    private fun upBookData() {
        books?.let { books ->
            ...
            val searchFiltered = if (searchKey.isNullOrEmpty()) {
                books
            } else {
                books.filter {
            ...
            adapter.setItems(
                BookFilter.apply(searchFiltered, bookFilterConfig) { it }
            )
            itemTouchCallback.isCanDrag =
                AppConfig.getBookSortByGroupId(viewModel.groupId) == 3 && !bookFilterConfig.isActive
        }
    }
    ...
    private fun upMenu() {
        menu?.findItem(R.id.menu_book_group)?.subMenu?.let { subMenu ->
            ...
            groupList.forEachIndexed { index, bookGroup ->
                ...
                subMenu.add(R.id.menu_group, itemId, index, bookGroup.groupName)
            }
        }
    }

    private fun showBookFilter() {
        BookFilterDialog.show(this, MANAGE_FILTER_PREF) {
            bookFilterConfig = it
            menu?.findItem(R.id.menu_book_filter)?.isChecked = it.isActive
            upBookData()
        }
    }
    ...
    companion object {
        private const val MANAGE_FILTER_PREF = "bookshelfManageBookFilter"
    }
